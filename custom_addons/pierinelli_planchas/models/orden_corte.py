...
medidas de los cortes y la modulacion (PDFs de AutoCAD) anexa. Al ejecutar
el corte, la plancha se divide en tres destinos:
...
class OrdenCorte(models.Model):
    ...
    plancha_id = fields.Many2one(
        'stock.lot', string='Plancha a cortar', required=True,
        domain="[('m2_disponible', '>', 0)]", tracking=True)
    ...
    sale_order_id = fields.Many2one(
        ...
        help='Pedido que origina el corte. Lo cortado queda en la plancha '
    ...
    linea_ids = fields.One2many(
        'pierinelli.orden.corte.linea', 'orden_id', string='Cortes')
    m2_cortes = fields.Float(
        'm² a cortar (venta)', compute='_compute_m2', store=True,
        digits=(8, 2))
    ...
    m2_retorno = fields.Float(
        'm² retorno a stock', compute='_compute_m2', store=True,
        digits=(8, 2))
    ...
    destino_merma = fields.Selection(
        ...
        string='Destino de la merma', default='negocio', required=True)
    ...
    @api.depends('sale_order_id', 'plancha_id')
    def _compute_partner(self):
        for orden in self:
            orden.partner_id = (orden.sale_order_id.partner_id
                                or orden.plancha_id.cliente_reserva_id
                                or orden.partner_id)

    @api.depends('linea_ids.m2', 'retorno_largo', 'retorno_alto',
                 'plancha_id.m2_disponible', 'state')
    def _compute_m2(self):
        for orden in self:
            orden.m2_cortes = round(sum(orden.linea_ids.mapped('m2')), 2)
            orden.m2_retorno = round(
                (orden.retorno_largo or 0) * (orden.retorno_alto or 0), 2)
            if orden.state == 'hecho':
                # ya ejecutada: la merma quedo fijada, no se recalcula
                continue
            orden.m2_merma = round(
                (orden.plancha_id.m2_disponible or 0)
                - orden.m2_cortes - orden.m2_retorno, 2)

    @api.constrains('linea_ids', 'retorno_largo', 'retorno_alto')
    def _check_m2(self):
        for orden in self.filtered(lambda o: o.state == 'borrador'):
            if orden.m2_merma < -0.01:
                raise ValidationError(_(
                    'Los cortes (%(c).2f m²) + el retorno (%(r).2f m²) '
                    'superan lo disponible de la plancha (%(d).2f m²).',
                    c=orden.m2_cortes, r=orden.m2_retorno,
                    d=orden.plancha_id.m2_disponible))
    ...
    def action_confirmar(self):
        ...
        for orden in self:
            if orden.state != 'borrador':
                continue
            plancha = orden.plancha_id
            if not orden.linea_ids:
                raise UserError(_('Agrega al menos un corte con sus medidas.'))
            if orden.m2_merma < -0.01:
                raise UserError(_('Los cortes superan lo disponible.'))
            ...
            if m2_retorno > 0.005:
                hija = self.env['stock.lot'].create({
                    ...
                    'largo': orden.retorno_largo,
                    'alto': orden.retorno_alto,
                ...
                Quant._update_available_quantity(
                    plancha.product_id, ubicacion, -m2_retorno,
                    lot_id=plancha)
                Quant._update_available_quantity(
                    plancha.product_id, ubicacion, m2_retorno,
                    lot_id=hija)
                orden.plancha_retorno_id = hija
            ...
            if m2_merma > 0.005:
                Quant._update_available_quantity(
                    plancha.product_id, ubicacion, -m2_merma,
                    lot_id=plancha)
                Quant._update_available_quantity(
                    plancha.product_id, orden._location_mermas(),
                    m2_merma, lot_id=plancha)
                self.env['pierinelli.merma'].create({
                    'plancha_id': plancha.id,
                    'orden_corte_id': orden.id,
                    'm2': m2_merma,
                    'motivo': 'corte',
                    'destino': orden.destino_merma,
            ...
            plancha.message_post(body=_(
                ...
                op=orden.name, v=orden.m2_cortes, r=m2_retorno,
                m=max(m2_merma, 0.0),
                dest=dict(orden._fields['destino_merma'].selection)
                    [orden.destino_merma]))
            orden.state = 'hecho'
            orden.m2_merma = max(m2_merma, 0.0)
        return True
